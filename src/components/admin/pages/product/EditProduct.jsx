...
import { updateProduct, getOneProduct } from "../../../../utils/hepperApi";
...
const EditProduct = () => {
    ...
    console.log(typeof productStatus)
    const id = useParams();
    const validation = () => {
        const msg = {}
    ...
    const handleSubmit = (e) => {
        ...
        oldImages.forEach(oldImage => {
            formData.append("old_images", oldImage);
        })
    ...
    useEffect(() => {
        getOneProduct(id).then((res) => {
            setProductName(res.data.message.product_name);
            setProductDesc(res.data.message.product_desc);
            setProductPrice(res.data.message.product_price);
            setProductListedPrice(res.data.message.product_listed_price);
            setProductQty(res.data.message.product_qty);
            setProductCategory(res.data.message.category_id);
            setProductBrand(res.data.message.brand_id);
            setProductStatus(res.data.message.product_status);
            setProductOrigin(res.data.message.product_origin);
            setProductCode(res.data.message.product_code);
            setUploadImage(`${serverPublic}images/product/${res.data.message.product_image_1}`);
            setOldImages([res.data.message.product_image_1, res.data.message.product_image_2, res.data.message.product_image_3]);
        });
    ...
    return (
        <div className="w-full">
            ...
            <div className="w-full flex flex-col items-center justify-center gap-4 mt-20 relative">
                ...
                <h4 className="text-textColor text-xl font-semibold">Sửa Sản Phẩm</h4>
